...
一个融合 LLM 智能派单、SLA 时效监控、多角色协作的现代化宿舍报修平台
...
| 🔴 人工分配维修员，效率低、不公平 | ✅ AI 智能派单，LLM 综合分析技能、负载、评分自动分配 |
...
- ✅ 审批/驳回/指派工单，修改优先级，自由切换所有状态
- 👥 用户管理 — 创建维修员/管理员账号（含手机号、技能选择），编辑技能标签
- 🔩 配件库存 CRUD，物料消耗排行
- 🤖 AI 引擎配置中心（支持 Ollama / DeepSeek / SiliconFlow / 通义千问 / 月之暗面等）
- 📢 公告发布与管理
- 📥 一键导出 CSV 报表

### 🤖 AI 智能能力
- **LLM 智能派单** — 大模型综合分析报修描述、维修员技能、负载、评分，输出最佳分配方案，失败自动降级规则引擎
- **AI 报修分类** — 大模型分析报修描述自动分类（水电/电器/家具/网络）并评估优先级
- **宿宝助手** — 内置 AI 聊天机器人，支持多模型配置（Ollama / DeepSeek / SiliconFlow / 通义千问等）
...
| **AI 能力** | 统一 LLM 调用（Ollama / DeepSeek / SiliconFlow / 通义千问等）+ 关键词规则引擎 |
...
生成模拟测试数据（30+ 报修记录、多角色账号）：

| 角色 | 邮箱 | 密码 |
|------|------|------|
| 管理员 | zhangsan@qq.com | 123456 |
| 管理员 | admin@example.com | admin123 |
| 学生 | xiaoming@qq.com | 123456 |
| 维修员（水电） | tech_li@qq.com | 123456 |
| 维修员（电器+家具） | tech_wang@qq.com | 123456 |
...
- 工单管理（状态筛选 + 审批/指派/驳回，自由切换所有状态）
- 用户管理（创建维修员/管理员 + 技能标签编辑 + 手机号）
- AI 配置中心（多引擎切换 + 连接测试，支持国内外主流模型）
...
1. **LLM 驱动的智能派单** — 大模型综合分析技能匹配、工作负载、历史评分，输出最佳分配方案，规则引擎兜底
2. **统一 AI 调用层** — `call_active_llm()` 复用 `ai_configs` 配置，派单、分类、聊天共用同一套模型，支持 Ollama / DeepSeek / SiliconFlow 等
3. **SLA 后台自动巡检** — 异步任务每 60 秒检测，多级预警通知，企业级运维思维
4. **安全加固** — 注册仅允许学生角色、库存原子扣减防竞态、JWT 密钥自动生成、维修员/管理员由管理员创建
5. **状态机硬约束** — 后端严格校验状态流转路径，管理员可自由切换所有状态
6. **配件库存联动** — 维修消耗原子扣减库存，管理员可追踪物料成本
